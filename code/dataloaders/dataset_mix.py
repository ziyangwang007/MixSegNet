...
class BaseDataSets(Dataset):
    def __init__(self, base_dir=None,labeled_type="labeled", split='train', transform=None, fold="fold1"):
        ...
        dense_labeled_ids=["patient{:0>3}".format(
            10 * i) for i in range(1,11,2)]
        sparse_labeled_ids=["patient{:0>3}".format(
            i) for i in range(1,100,2)]
        if self.split == 'train':
            self.all_slices = os.listdir(
                self._base_dir + "/ACDC_training_slices")
            self.sample_list = []
            dense_label_ids = [i for i in dense_labeled_ids if i in train_ids]
            sparse_label_ids = [i for i in sparse_labeled_ids if i in train_ids]
            unlabeled_ids = [i for i in train_ids if i not in dense_label_ids and i not in sparse_label_ids]
            if self.labeled_type == "label":
                print("Dense labeled patients IDs", dense_label_ids)
                for ids in dense_label_ids:
                    new_data_list = list(filter(lambda x: re.match(
                        '{}.*'.format(ids), x) != None, self.all_slices))
                    self.sample_list.extend(new_data_list)
                print("Total dense labeled {} samples".format(len(self.sample_list)))
            elif self.labeled_type == "scribble":
                print("Sparse labeled patients IDs", sparse_label_ids)
                for ids in unlabeled_ids:
                    new_data_list = list(filter(lambda x: re.match(
                        '{}.*'.format(ids), x) != None, self.all_slices))
                    self.sample_list.extend(new_data_list)
                print("Total sparse labeled {} samples".format(len(self.sample_list)))
            else:
